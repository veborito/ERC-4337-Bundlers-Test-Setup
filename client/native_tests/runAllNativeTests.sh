...
for config in "${TEST_CONFIGS[@]}"; do
	...
	echo "=========================================="
	./runNativeTest.sh "$MACHINE" "test-${num}-$PROCESSOR-r${rounds}-s${sca}-t${throttle}-b${block_time}" "$rounds" "$sca" "$throttle" "$block_time"
	((num++))
	sleep 1
done

./runIdleNativeTest.sh "$MACHINE" "$PROCESSOR"
